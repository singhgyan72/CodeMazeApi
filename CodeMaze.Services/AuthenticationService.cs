...
namespace CodeMaze.Services
{
    internal sealed class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<IdentityResult> RegisterUser(UserForRegistrationDto userForRegistration)
        {
            ...
            //The CreateAsync method will save the user to the database if the action succeeds
            //or it will return error messages as a result.
            var result = await _userManager.CreateAsync(user, userForRegistration.Password);

            //if a user is created, we add that user to the named roles — the ones sent from the client side
            if (result.Succeeded)
